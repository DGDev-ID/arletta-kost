...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Signature', href: '/transactions/signatures' },
...
const statusBadge = (s: string) => {
    const map: Record<string, string> = {
        ...
        finished_payment: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
        down_payment:     'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400',
...
<template>
    <Head :title="`Detail Signature #${bill.id}`" />
            ...
            <div class="max-w-7xl mx-auto px-6 space-y-8">
                ...
                <div class="flex items-center gap-3">
                    <Button variant="ghost" size="icon" class="h-8 w-8" as-child>
                        <Link :href="route('transactions.signatures.index')">
                            <ArrowLeft class="h-4 w-4" />
                        </Link>
                    </Button>
                    <Heading :title="`Detail Bill #${bill.id}`" description="Detail tagihan dan tanda tangan penyewa." />
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="rounded-lg border bg-card p-6 space-y-4">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between border-b pb-2">
                                ...
                                <span :class="statusBadge(bill.status)" class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium">
                                    {{ statusLabel(bill.status) }}
                                </span>
                            </div>
                            <div class="flex justify-between border-b pb-2">
                                <span class="text-sm text-muted-foreground">Skema Pembayaran</span>
                                <span class="text-sm font-medium capitalize">{{ bill.payment_scheme.replace('_', ' ') }}</span>
                            </div>
                            <div class="flex justify-between border-b pb-2">
                            ...
                            <div v-if="bill.dp_amount" class="flex justify-between border-b pb-2">
                                <span class="text-sm text-muted-foreground">Jumlah DP</span>
                                <span class="text-sm font-medium">{{ formatCurrency(bill.dp_amount) }}</span>
                            </div>
                            <div class="flex justify-between border-b pb-2">
                    ...
                    <!-- Signature -->
                    <div class="rounded-lg border bg-card p-6 space-y-4 md:col-span-2">
                        <HeadingSmall title="Tanda Tangan" description="Tanda tangan digital penyewa." />
                        <div v-if="bill.signature" class="flex justify-center">
                            <div class="rounded-lg border bg-white p-4 inline-block">
                                <img :src="bill.signature" alt="Tanda Tangan" class="max-h-48 object-contain" />
                            </div>
                        </div>
                        <div v-else class="text-sm text-center py-6 text-muted-foreground">
                    ...
                    <div class="rounded-lg border bg-card p-6 space-y-4 md:col-span-2">
                        ...
                        <div v-if="transactions.length > 0" class="space-y-6">
                            <div v-for="t in transactions" :key="t.id" class="rounded-md border p-4">
                                <div class="flex flex-col sm:flex-row justify-between sm:items-center border-b pb-3 mb-3 gap-2">
                                    ...
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        <span class="inline-flex rounded-full bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 px-2 py-0.5 text-xs font-medium">
                                            {{ transactionTypeLabel(t.transaction_type) }}
                                        </span>
                                        <span :class="statusBadge(t.status)" class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium">
                                ...
                                <div>
                                    ...
                                    <ul class="relative border-l border-muted-foreground/30 ml-2 space-y-3">
                                        <li v-for="d in t.details" :key="d.id" class="ml-4">
                                            <div class="absolute w-2 h-2 bg-primary rounded-full -left-1 mt-1.5 border border-background"></div>
                                            <div class="flex justify-between items-start">
                                                <p class="text-sm font-medium capitalize">{{ statusLabel(d.status) }}</p>
